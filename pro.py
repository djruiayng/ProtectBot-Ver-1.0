...
cl = LINE("fckdvwxe@iwebtm.com","djry9420")
print("機器壹登入成功")
k1 = LINE("krpsisd@ymail365.com","djry9420")
print("機器貳登入成功")
k2 = LINE("twjzwgiks@net3mail.com","djry9420")
print("機器參登入成功")
k3 = LINE("oagdaoke@itymail.com","djry9420")
print("機器肆登入成功")
k4 = LINE("vwwecr@inwebtm.com","djry9420")
print("機器伍登入成功")
k5 = LINE("vejasif@ymail365.com","djry9420")
print("機器陸登入成功")
k6 = LINE("weqdkby@itymail.com","djry9420")
print("翻機登入成功")
print("[ 登錄系統 ]成功(  -᷄ω-᷅ )")
...
def lineBot(op):
    try:
        ...
        if op.type == 5:
            ...
            cl.sendMessage(op.param1, "你好 {} 謝謝你加我為好友 ε٩(๑> ₃ <)۶з \n此機器為戰爭防翻機器人 月租1張票300元整\n有興趣可以私以下友資購買".format(str(cl.getContact(op.param1).displayName)))
            cl.sendMessage(op.param1, None, contentMetadata={'mid': 'ub809d1621e65e5defd74e9e61b1b51b4'}, contentType=13)
        if op.type ==19:
            a = 0
            if op.param2 in ban["admin"] or op.param2 in ban["owners"]:
        ...
        if op.type == 17:
            if op.param2 in ban['blacklist']:
                bot = random.choice([k1,k2,k3,k4,k5,k6])
                bot.kickoutFromGroup(op.param1,[op.param2])                 
        ...
        if op.type == 26 or op.type == 25:
            ...
            if sender in sender:
                if text.lower() == 'gc':
                    ...
                    cl.sendMessage(to,"收收")
                    k1.sendMessage(to,"收.")
                    k2.sendMessage(to,"收!!!!")
                    k3.sendMessage(to,"水")
                    k4.sendMessage(to,"爽")
                    k5.sendMessage(to,"繼續")
                    k6.sendMessage(to,"收!!!!!!!")
                elif text.lower() =='跟':
                    cl.sendMessage(to,"跟喔")
                    k1.sendMessage(to,"跟!!!!!")
                    k2.sendMessage(to,"跟.")
                    k3.sendMessage(to,"跟了")
                    k4.sendMessage(to,"來")
                    k5.sendMessage(to,"開")
                    k6.sendMessage(to,"收起來!!!!!!!")
                ...
                if text.lower().startswith("gdel "):
                    ...
                    if settings["kickmeber"] == True:
                        if msg.toType == 2:
                            ...
                            gs = cl.getGroup(to)
                            start = time.time()
                            cl.sendMessage(to, "踹一腳只要0.000000185854464秒")
...
#                            cl.sendMessage(to, "🔱ㄊㄌ🔱")
                            ...
                            if targets == []:
                                cl.sendMessage(receiver,"沒有其他人")
                            else:
                                for target in targets:
                                   if not target in Rfu:
                                        try:
                                            time.sleep(0.0001)
                                            k6.kickoutFromGroup(to,[target])
                                            elapsed_time = time.time() - start
                                            cl.sendMessage(to, "已踢除完成\n踢除時間: %s秒" % (elapsed_time))
                                            time.sleep(3)
                                            cl.sendMessage(to, "バイバイ")
                                        except:
                                            pass
